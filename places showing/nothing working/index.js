...
var app = {
    ...
    initializeUI: function() {
    	...
    	$enrtiesPage.find("._checkTasksNearby").click(function(e) {
			...
			if (places.park == false && places.store == false && places.bank == false){
				alert("appears as though everything is empty");
			}else{
				
				alert("about to print the current loc");
				var myLocation = new google.maps.LatLng(geo.latitute,geo.longitude);
				
				//now checking by location type
				if(places.park == true){	
					var requestPark = { location: myLocation, radius: '1000', types: ['park'] }; 
					...
					var requestBank = { location: myLocation, radius: '1000', types: ['bank'] }; 
					infowindow = new google.maps.InfoWindow();

					var service = new google.maps.places.PlacesService(MapEl.map);
					service.nearbySearch(requestBank, callback);
				}else if(places.store == true){
					var requestStore = { location: myLocation, radius: '1000', types: ['bank'] }; 
					infowindow = new google.maps.InfoWindow();

					var service = new google.maps.places.PlacesService(MapEl.map);
					service.nearbySearch(requestStore, callback);
